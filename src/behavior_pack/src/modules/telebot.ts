...
FormFields.telebot = {
  x: formLib.slider(-5, 5, {displayName: 'X Offset', defaultValue: 0}),
  y: formLib.slider(-5, 5, {displayName: 'Y Offset', defaultValue: 0}),
  z: formLib.slider(-5, 5, {displayName: 'Z Offset', defaultValue: 0}),
  w: formLib.slider(0, 10, {displayName: 'Teleport Area Size', defaultValue: 1}),
}
...
async function TeleportUserToNpc(d: Discussion, npc: NpcState) {
  ...
  let y = npc.location[2] + ((settings.y as number) ?? 0);
  ...
  for (let i=5; i > 0; i--) {
    for (let j = 0; j < (i); j++) {
      ...
      dimension.spawnParticle("minecraft:endrod", newLocation, new mc.MolangVariableMap());
    }
    await timeout(5);
  }
}
