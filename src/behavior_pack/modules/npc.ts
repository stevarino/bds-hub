...
import {NpcInitiated} from '../lib/runtime_state.js';
...
async function editBot(d: Discussion, isAdmin: boolean, npc: NpcState) {
  ...
  if (isAdmin) {
    if (results.delete.get()) {
      await STATE.rmNpc(npc.id);
      const success = await loadNpc(npc.location, npc.id, entity => {
        entity.remove();
      });
      if (!success) await showErrorMessage(d.player, 'Failed to kill bot.');
    }
