...
const PLAYER_LOCATIONS = new DefaultMap(() => new Set<number>());
    ...
    if (settings.getPlayerBit(player, LocationShown, true)) {
      ...
      for (const locId of PLAYER_LOCATIONS.get(player.id)) {
        ...
        const color = colorNames[
          LocationColor[loc.color] as keyof typeof colorNames
        ] as string | undefined;
...
async function NewLocation(d: Discussion, args: Args) {
  ...
  const res = await LocNew.request(req);
  if (res?.success !== true) {
...
async function EditLocation(d: Discussion, args: Args) {
  ...
  if (loc === undefined) {
    return showErrorMessage(d.player, 'Unable to find location');
  }
  ...
  if (results.delete.value === true) {
    const res = await showDialogMessage(
      d.player,
      'Confirm Deletion',
