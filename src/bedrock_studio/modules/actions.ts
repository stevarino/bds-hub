...
import {Events} from '../../types/requests.js';
...
async function Menu(d: Discussion, args: types.Args) {
  ...
  for (const btn of menu.buttons) {
    if (btn.require_tag !== undefined && !d.player.hasTag(btn.require_tag))
      continue;
  ...
  return d.handleTransition(action);
}

async function Time(d: Discussion): Promise<void | undefined> {
  const cnt = d.get('timeActions', 0) + 1;
...
async function InventoryInspect(d: Discussion) {
  const component = d.player.getComponent('minecraft:inventory') as
    | mc.EntityInventoryComponent
    | undefined;
  ...
  for (let i = 0; i < component.inventorySize; i++) {
    ...
    if (enchantments !== undefined) {
      for (const e of enchantments.getEnchantments()) {
        let enchantment = (typeof e.type === 'string') ? e.type : e.type.id;
        enchantmentList[enchantment] = e.level;
      }
    }
    items.push(
      JSON.stringify(
 ...
 */
async function Teleport(d: Discussion) {
  const targets = [];
  ...
  const res = await lib.getFormResponse(d.player, form);
  if (res.selection === undefined) return;
  await d.player.dimension.runCommand(
    `teleport "${d.player.name}" ${targets[res.selection]}`,
  );
}
...
async function Give(d: Discussion, args: types.Args) {
  ...
  const enchants = stack.getComponent(mc.ItemComponentTypes.Enchantable);
  if (giveArgs.lore !== undefined) stack.setLore(giveArgs.lore);
  if (giveArgs.name !== undefined) stack.nameTag = giveArgs.name;
  d.player.dimension.spawnItem(stack, d.player.location);
  for (const [enchantment, level] of Object.entries(
    giveArgs.enchantments ?? {},
  )) {
    enchants?.addEnchantment({type: enchantment, level});
  }
}
...
function getLines(result?: Partial<types.Event>[]) {
  ...
  function justify(row: {object?: string; qty?: number}) {
    return (
      lib.padToWidth(
        (row.object ?? 'error').replace('minecraft:', '') + ': ',
        146,
        '.',
