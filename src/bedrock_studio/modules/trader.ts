...
import {showErrorMessage, forms, showDialogMessage, strip} from '../../lib/lib.js';
import {
  TradeArray,
  TradeItem,
  TradeOffer,
...
import {actionCallback, defineActions, Discussion, Args} from './discussion.js';
...
async function Trader(d: Discussion, args: Args) {
  ...
  if (!browsing && Math.max(...qts) === 0) {
    const option = await showDialogMessage(
      d.player,
      'No Trades Available',
  ...
  for (let i = 0; i < targs.trades.length; i++) {
    ...
    const trade = targs.trades[i]!;
    buttons.push({
      text: tradeText(trade),
      action: async () => {
        await d.action(browsing ? TraderBrowse.name : TraderOffer.name, trade, {
          _traderArgs: targs,
          _browsing: browsing,
        });
      },
  ...
  await forms.ActionForm(
    d.player,
    browsing ? 'View a Trade' : 'Make a Trade',
    `\n${msg}\n`,
    buttons,
  );
}
...
async function TraderOffer(d: Discussion, args: Args) {
  ...
  const get = offer.gives
    .map(item => `- ${itemString(item.item)} (${item.qty ?? 1})`)
    .join('\n');

  const cost = trade.inventory
    .map(
      item =>
        `- ${itemString(item.item)} (${item.cost ?? 1} / ${item.qty ?? 1})`,
    )
    .join('\n');
  ...
  const buttons: forms.ActionButton[] = [
    {
      text: 'Trade',
      action: async () => {
        await performTrade(d.player, offer, trade, false);
  ...
  if (trade.maxTrades > 1)
    buttons.push({
      text: `Trade All (${trade.maxTrades})`,
      action: async () => {
        await performTrade(d.player, offer, trade, true);
        await d.action(TraderOffer.name, args);
      },
    });
  buttons.push({
...
async function TraderBrowse(d: Discussion, args: Args) {
  ...
  for (const accept of offer.accepts) {
    costs.push(accept.map(item => `- ${item.item} (${item.qty})`).join('\n'));
  }
...
function performTrade(
  player: mc.Player,
  offer: NormalizedTradeOffer,
  op: InventoryOperation,
  all: boolean,
) {
  const container = (
    player.getComponent('minecraft:inventory') as mc.EntityInventoryComponent
  ).container;
  if (container === undefined) return;
  const mutations: [slot: number, qty: number][] = [];
  for (const item of op.inventory) {
    ...
    for (const slot of item.slots) {
      ...
      if (stack?.typeId !== item.item) {
        return showErrorMessage(
          player,
          `Unable to complete trade. Invalid inventory item: ${JSON.stringify({
            received: stack?.typeId,
            expteced: item.item,
            slot,
          })}`,
        );
    ...
    if (need !== 0) {
      return showErrorMessage(
        player,
        `Unable to complete trade. Missing items: ${JSON.stringify({
          expteced: item.item,
          need,
          mutations,
        })}`,
  ...
  for (const {item, qty} of offer.gives) {
    container.addItem(
      new ItemStack(item, (qty ?? 1) * (all ? op.maxTrades : 1)),
    );
  }
  return undefined;
}
...
function tradeQuantitiesForOffer(
  inventory: InventoryMap,
  offer: NormalizedTradeOffer,
) {
...
function tradeText(offer: NormalizedTradeOffer) {
  ...
  return `${itemString(first.item)}${qty > 1 ? ` (${qty})` : ''}${
