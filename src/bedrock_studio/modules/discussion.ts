...
import {script as data} from '../data.js';
import * as types from '../../types/common.js';
import * as formlib from '../../lib/form.js';
import {DIMENSION} from '../../lib/constants.js';
import {getFormResponse, showErrorMessage, timeout} from '../../lib/lib.js';
export {Args, Action} from '../../types/common.js';
...
export function actionCallback(
...
export class Discussion {
  ...
  findNpc() {
    ...
    for (const dim of Object.values(DIMENSION)) {
      try {
        npcs.push(
          ...mc.world.getDimension(dim).getEntities({tags: [this.npc]}),
        );
      } catch {
  ...
  findTransition(transitionName: string): types.Transition | undefined {
    const transition = data.transitions[transitionName];
...
async function check<T>(
  ...
  type: keyof types.Transition,
  handler: (d: Discussion, action: T) => void | Promise<void>,
) {
...
async function respondAction(d: Discussion, args: types.Action) {
  ...
  const customAction = data.actions[args.action];
