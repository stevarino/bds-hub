...
lib.StartupEvent.addListener(startup);
...
regions.onEnter.addListener(onLocationEnter);
regions.onExit.addListener(onLocationLeave);
...
const LOCATION_EVENT = 'LOCATION_MODULE';
...
const PLAYER_LOCATIONS = new lib.DefaultMap(() => new Set<number>());
...
function onTick() {
  ...
  for (const player of mc.world.getAllPlayers()) {
    if (settings.getPlayerBit(player, LocationShown, true)) {
      ...
      for (const locId of PLAYER_LOCATIONS.get(player.id)) {
        const loc = LOCATIONS.get(locId)!;
        if (!loc.isPublic && loc.owner === player.name) continue;
        const color = colorNames[
          LocationColor[loc.color] as ColorKey
        ] as string | undefined;
      ...
      text.addActionBarText({id: `loc:${player.name}`});
    }
  }
}

async function updateLocations() {
  const locs = await reqTypes.LocGetAll.request();
  ...
  for (const l of locs.locations) {
    ...
    regions.registerRegion({
      ...
      id: l.id,
      pos1: {x: l.x1, y: l.y1, z: l.z1},
      pos2: {x: l.x2, y: l.y2, z: l.z2},
    });
  }
  return true;
}
...
async function SelectLocation(d: Discussion, args: Args) {
  ...
  const buttons: forms.ActionButton[] = [
    {
      ...
      action: actionCallback(d, NewLocation.name, args),
    },
  ];
  ...
  const locs = await reqTypes.LocList.request({owner: args.owner as string | undefined});
  if (locs !== undefined) {
...
async function NewLocation(d: Discussion, args: Args) {
  ...
  const size =
    results.size.get() === DEFAULT_SIZE ? 0 : parseInt(results.size.get());
  ...
  try {
    req = {
      ...
      type: LocationType[results.type.get() as LocationTypeKey],
      color: LocationColor[results.color.get() as ColorKey],
      sort: DEFAULT_SORT,
      isPublic: results.publiclyViewable.get(),
    };
  } catch (e) {
    await lib.showErrorMessage(d.player, String(e));
    return;
  }
  const res = await reqTypes.LocNew.request(req);
  if (res?.success !== true) {
...
async function EditLocation(d: Discussion, args: Args) {
  ...
  const loc = (await reqTypes.LocGet.request({id: args.locId as number})).location;
  if (loc === undefined) {
    return lib.showErrorMessage(d.player, 'Unable to find location');
  }
  ...
  const {results} = await forms.ModalForm(d.player, title, {
    ...
    y1: forms.textbox('', {
      defaultValue: String(loc.y1 ?? d.player.location.y),
    }),
    y2: forms.textbox('', {
      defaultValue: String(loc.y2 ?? d.player.location.y),
    }),
  ...
  if (results.delete.value === true) {
    const res = await lib.showDialogMessage(
      d.player,
      'Confirm Deletion',
    ...
    if (res === 0) {
      const res = await reqTypes.LocDel.request({id: args.locId as number});
      await lib.showDialogMessage(
        d.player,
        title,
        res?.success === true
          ? 'Success'
  ...
  const res = await reqTypes.LocUpdate.request(req);
  if (res?.success !== true) {
    await lib.showDialogMessage(
      d.player,
      title,
...
function onLocationEnter(e: regions.OnRegionChangeEvent): void {
  if (e.source !== LOCATION_EVENT) return;
  PLAYER_LOCATIONS.get(e.player.id).add(e.index!);
}

function onLocationLeave(e: regions.OnRegionChangeEvent): void {
  if (e.source !== LOCATION_EVENT) return;
  PLAYER_LOCATIONS.get(e.player.id).delete(e.index!);
}
