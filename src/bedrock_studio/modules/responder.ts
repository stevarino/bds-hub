...
import {Discussion, data} from './discussion.js';
import './settings.js';
import './actions.js';
import './npc.js';
import './locations.js';
import './minigames/main.js';
import '../../lib/storage.js';
import './trader.js';
import './telebot.js';
import './item_durability.js';
import {SuperItemUse, Transition} from '../../types/common.js';
...
world.afterEvents.itemUse.subscribe(e => {
  ...
  discussions[e.source.name]?.navigate(itemUsed).catch(lib.showErrorTraceback);
});
...
system.afterEvents.scriptEventReceive.subscribe(e => {
  if (e.id === 'hub:dialogue_transition') {
    getDiscussion(e.initiator as Player)
      .go(e.message)
  ...
  if (e.id === 'hub:npc_interact') {
    ...
    if (players.length !== 1) {
      console.error(
        `Unable to find singular player for "${playerName}" : `,
    ...
    const actorDef = data.actors[actor];
  ...
  if (e.id === 'hub:log') {
    ...
    console.info(
      ...
      JSON.stringify(
        {
          initiator:
            player?.name ?? e.initiator?.nameTag ?? e.initiator?.typeId ?? null,
          sourceType: e.sourceType,
          sourceEntity:
            e.sourceEntity?.nameTag ?? e.sourceEntity?.typeId ?? null,
        },
        undefined,
        2,
      ),
