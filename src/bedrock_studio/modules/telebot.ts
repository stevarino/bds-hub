...
async function NpcTravel(d: Discussion) {
  ...
  for (const npc of npcs.NPCs.getAll()) {
    const actor = data.actors[npc.role];
    if (actor === undefined) {
  ...
  if (buttons.length === 0) {
    return await lib.showErrorMessage(
      d.player,
...
async function TeleportUserToNpc(d: Discussion, npc: npcs.NpcState) {
