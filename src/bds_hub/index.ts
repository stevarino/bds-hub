...
(async () => {
  network.setHost(storage.WORLD.get('host') as string);
  await poll();
...
async function poll() {
  ...
  for (const p of mc.world.getAllPlayers()) {
    ...
    update.pos = [
      p.dimension.id,
      Math.round(p.location.x),
      Math.round(p.location.y),
      Math.round(p.location.z),
    ];
  }
  ...
  try {
    res = await network.CheckIn.request(copy);
  } catch (e) {
  ...
  if (res === undefined) {
    // 2147954429 on error?
    console.error(
      `Unable to contact ${HOST}, sleeping for ${
        Math.round((10 * ticksPerPoll) / 20) / 10
      }s`,
    );
    return;
  }
...
mc.world.afterEvents.weatherChange.subscribe(e => {
  PAYLOAD.weather = e.lightning
    ? types.Constants.weatherLightning
    : e.raining
    ? types.Constants.weatherRain
    : types.Constants.weatherClear;
});

mc.world.afterEvents.chatSend.subscribe(e => {
  // don't broadcast whisper or other player-specific messages
  if (e.message.startsWith('!hub')) {
    return parseCommand(e);
  }
  if (e.targets) return;
  PAYLOAD.messages.push(`**<${e.sender.name}>** ${e.message}`);
});

mc.system.afterEvents.scriptEventReceive.subscribe(async e => {
  if (e.id !== 'hub:request') return;
  const req = JSON.parse(e.message) as network.EndpointTransaction;
  const res: network.EndpointTransaction = {
    id: req.id,
    endpoint: req.endpoint,
    payload: await network.requestRaw(e.id.split(':')[1]!, req.payload)
  };
  mc.world.getDimension(lib.constants.DIMENSION.OVERWORLD).runCommand(
    `scriptevent hub:response ${JSON.stringify(res)}`
  )
}, {namespaces: ['hub']});

function parseCommand(e: mc.ChatSendAfterEvent): void {
