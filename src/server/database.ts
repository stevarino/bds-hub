...
type DBLocationResult = Omit<LocationResult, 'dimension' | 'owner'> & {
  dimension: number;
  owner?: number;
};
...
export async function openDatabase(filename: string) {
  ...
  await db.migrate({
    migrationsPath: join(root, 'static', 'server', 'migrations'),
  });

  // cache all the things
  const strings: StringLookup[] = await db.all('SELECT id, value FROM Strings');
...
export class DBHandle {
  ...
  formatParams<T = Obj<string> | Obj<number>>(params: T): T {
    return Object.fromEntries(
      Object.entries(params as object).map(([k, v]) => [
        k.startsWith(':') ? k : ':' + k,
        v ?? null,
      ]),
    ) as T;
  ...
  async stringsToIds<T extends {[key: string]: string | undefined}>(
    strings: T,
  ): Promise<{[U in keyof T]: number}> {
    ...
    let query = `INSERT OR IGNORE INTO Strings(value) VALUES ${values
      .map(() => '(?)')
      .join(', ')};`;
    await this.db.run(query, values);
    query = `SELECT id, value FROM "Strings" WHERE ${values
      .map(() => 'value = ?')
      .join(' OR ')};`;
    const rows = await this.db.all<{id: number; value: string}[]>(
      query,
      values,
    );
  ...
  async addPosition(
    ts: number,
    player: string,
    position: Dialogue.PositionTuple,
  ) {
  ...
  async queryEvents(query?: EventRequest) {
    ...
    for (const [key, value] of Object.entries(query?.where ?? {})) {
      ...
      params.push(
        isNum ? Number(value) : this.revCache.get(value as string) ?? 0,
      );
    }
    const qry = `SELECT entity, action, object, extra, qty FROM Events WHERE 1 = 1 ${where.join(
      ' ',
    )};`;
    ...
    for (const value of values) {
      ...
      for (const [k, v] of Object.entries(value)) {
        if (k === 'action') {
          ...
          record[k as Exclude<EventField, 'qty' | 'action'>] =
            this.cache.get(v);
        }
      }
    ...
    results.sort((a, b) => {
      for (let f of (query?.order ?? []) as string[]) {
        ...
        if ((a[f as EventField] ?? '') < (b[f as EventField] ?? ''))
          return -1 * rev;
        if ((a[f as EventField] ?? '') > (b[f as EventField] ?? ''))
          return 1 * rev;
      }
  ...
  async getKey(key: string) {
    const result = await this.db.get<{value: string}>(
      ...
      this.formatParams({key}),
    );
    return result?.value;
  }

  async listLocations(owner?: string, publicOnly = false) {
    const where = [];
    const params = {};
    ...
    for (const row of rows) {
      const update: {owner?: string; dimension: string} = {
  ...
  async createLocation(location: Location) {
    const loc: DBLocation = Object.assign(
      {},
      location,
      await this.stringsToIds({
  ...
  normalizeLocation(loc: DBLocation): Location {
    const updates: {dimension: string; owner?: string} = {
      dimension: this.cache.get(loc.dimension) as string,
    };
  ...
  async deleteLocation(locationId: number) {
    return (await this.db.run(`DELETE FROM Locations WHERE id = ${locationId}`))
      .changes;
  }
