...
async function buildSupPack(filename: string) {
  ...
  const file = JSON.parse(fs.readFileSync(filename, 'utf-8')) as HubSupplemental;
  for (const [entity_id, options] of Object.entries(file.npc_entities ?? {})) {
    ...
    for (let i=0; i<Object.keys(textureMap).length; i++) {
      events[`hub:npc_skin_${i}`] = { add: { component_groups: [ `hub:npc_skin_${i}`]}};
    ...
    const overrides: Overrides = {
      ...
      npc_rp_entity: [
        { path: 'minecraft:client_entity.description.identifier', value: entity_id },
        { path: 'minecraft:client_entity.description.textures', value: textureMap },
        { 
    ...
    for (const fileTemplate of Object.keys(CONFIG_TEMPLATES)) {
      ...
      const template = path.join(
        lib.root, 'static', 'lib', 'hubBuildSup', fileTemplate + '.json');
...
async function mergeTextures(packDir: string, safe_entity_id: string, options: NpcOptions) {
  const textures = options.texture_map ?? {}
  if (options.textures !== undefined) {
    const finalDir = fs.realpathSync(path.join(
      packDir, 'rp', 'textures', 'entity', safe_entity_id));
...
function performOverrides(json: any, overrides: (Override[]|undefined)[]) {
  for (const override of overrides) {
    for (const rule of override ?? []) {
      ...
      let parent = JSONPath({ path: JSONPath.toPathString(path), json: (json as object), wrap: false });
      if (parent === undefined) {
        let target: any = json
